...
namespace Piccolo
{
    ...
    void BlurPass::setupPipelines()
    {
        ...
        RHIShader* vert_shader_module = m_rhi->createShaderModule(BLUR_VERT);
        RHIShader* frag_shader_module = m_rhi->createShaderModule(BLUR_FRAG);
    ...
    void BlurPass::updateAfterFramebufferRecreate(RHIImageView* input_attachment)
    {
        ...
        RHIDescriptorImageInfo blur_LUT_image_info = {};
        blur_LUT_image_info.sampler = m_rhi->getOrCreateDefaultSampler(Default_Sampler_Linear);
        blur_LUT_image_info.imageView =
            m_global_render_resource->_color_grading_resource._color_grading_LUT_texture_image_view;
        blur_LUT_image_info.imageLayout = RHI_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
        ...
        post_process_descriptor_LUT_write_info.pImageInfo            = &blur_LUT_image_info;
    ...
    void BlurPass::draw()
    {
        ...
        m_rhi->pushEvent(m_rhi->getCurrentCommandBuffer(), "Blur", color);
